# purocean/yn

[![Stars](https://img.shields.io/github/stars/purocean/yn?style=flat-square&color=yellow)](https://github.com/purocean/yn/stargazers) [![Forks](https://img.shields.io/github/forks/purocean/yn?style=flat-square&color=blue)](https://github.com/purocean/yn/network) [![Language](https://img.shields.io/badge/lang-TypeScript-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-99%2F100-brightgreen?style=flat-square)](#)

> A highly extensible Markdown editor. Version control, AI Copilot, mind map, documents encryption, code snippet running, integrated terminal, chart embedding, HTML applets, Reveal.js, plug-in, and macro replacement.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 6.6k |
| 🍴 **Forks** | 930 |
| 💻 **Language** | TypeScript |
| 📈 **Score** | 99/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

AI/ML · Frontend · Security

## 📝 Summary

The open-source project "yn" is a highly customizable Markdown editor that offers a wide range of features to enhance productivity and security. With capabilities such as version control, AI-powered writing assistance, and document encryption, it serves as a versatile tool for individuals and teams. Its extensibility and integrations with various technologies make it an ideal choice for developers, writers, and knowledge workers.

---

<sub>🔭 Discovered 2026-05-01 · [View on GitHub](https://github.com/purocean/yn) · [← Back to AI/ML](./README.md)</sub>
