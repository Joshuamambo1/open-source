# WagnerAgent/awesome-mcp-servers-devops

[![Stars](https://img.shields.io/github/stars/WagnerAgent/awesome-mcp-servers-devops?style=flat-square&color=yellow)](https://github.com/WagnerAgent/awesome-mcp-servers-devops/stargazers) [![Forks](https://img.shields.io/github/forks/WagnerAgent/awesome-mcp-servers-devops?style=flat-square&color=blue)](https://github.com/WagnerAgent/awesome-mcp-servers-devops/network) [![Language](https://img.shields.io/badge/lang-Unknown-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-47%2F100-brightgreen?style=flat-square)](#)

> A curated, DevOps-focused list of Model Context Protocol (MCP) servers—covering source control, IaC, Kubernetes, CI/CD, cloud, observability, security, and collaboration—with a bias toward maintained, production-ready integrations.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 93 |
| 🍴 **Forks** | 33 |
| 💻 **Language** | Unknown |
| 📈 **Score** | 47/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

AI/ML · Backend · DevOps/Infra

## 📝 Summary

The awesome-mcp-servers-devops project is an open-source, curated list of Model Context Protocol (MCP) servers optimized for production-ready DevOps integrations. This comprehensive collection covers a wide range of tools, including source control, infrastructure-as-code, Kubernetes, and more, to streamline MCP server management. It prioritizes maintained and production-ready integrations, making it a valuable resource for developers and DevOps teams.

---

<sub>🔭 Discovered 2026-04-25 · [View on GitHub](https://github.com/WagnerAgent/awesome-mcp-servers-devops) · [← Back to AI/ML](./README.md)</sub>
