# supabase/supabase

[![Stars](https://img.shields.io/github/stars/supabase/supabase?style=flat-square&color=yellow)](https://github.com/supabase/supabase/stargazers) [![Forks](https://img.shields.io/github/forks/supabase/supabase?style=flat-square&color=blue)](https://github.com/supabase/supabase/network) [![Language](https://img.shields.io/badge/lang-TypeScript-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-100%2F100-brightgreen?style=flat-square)](#)

> The Postgres development platform. Supabase gives you a dedicated Postgres database to build your web, mobile, and AI applications.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 101.6k |
| 🍴 **Forks** | 12.2k |
| 💻 **Language** | TypeScript |
| 📈 **Score** | 100/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

AI/ML · Frontend · Data

## 📝 Summary

Supabase is an open-source Postgres development platform that offers a dedicated database for building web, mobile, and AI applications. It provides a powerful foundation for data-driven projects, enabling developers to focus on innovation without the hassle of database management. With Supabase, users can create scalable and secure applications with ease.

---

<sub>🔭 Discovered 2026-04-24 · [View on GitHub](https://github.com/supabase/supabase) · [← Back to AI/ML](./README.md)</sub>
