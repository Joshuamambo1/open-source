# microsoft/finnts

[![Stars](https://img.shields.io/github/stars/microsoft/finnts?style=flat-square&color=yellow)](https://github.com/microsoft/finnts/stargazers) [![Forks](https://img.shields.io/github/forks/microsoft/finnts?style=flat-square&color=blue)](https://github.com/microsoft/finnts/network) [![Language](https://img.shields.io/badge/lang-R-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-53%2F100-brightgreen?style=flat-square)](#)

> Microsoft Finance Time Series Forecasting Framework (FinnTS) is a forecasting package that utilizes cutting-edge time series forecasting and parallelization on the cloud to produce accurate forecasts for financial data.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 255 |
| 🍴 **Forks** | 50 |
| 💻 **Language** | R |
| 📈 **Score** | 53/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

AI/ML · Data

## 📝 Summary

FinnTS is a cloud-based time series forecasting framework developed by Microsoft, utilizing cutting-edge AI and ML techniques to provide accurate financial data forecasts. This open-source package leverages parallelization capabilities to efficiently process large datasets, enabling users to make data-driven decisions with confidence. By harnessing the power of cloud computing, FinnTS empowers users to improve forecasting accuracy and streamline financial planning processes.

---

<sub>🔭 Discovered 2026-04-29 · [View on GitHub](https://github.com/microsoft/finnts) · [← Back to AI/ML](./README.md)</sub>
