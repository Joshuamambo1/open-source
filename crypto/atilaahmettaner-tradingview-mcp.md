# atilaahmettaner/tradingview-mcp

[![Stars](https://img.shields.io/github/stars/atilaahmettaner/tradingview-mcp?style=flat-square&color=yellow)](https://github.com/atilaahmettaner/tradingview-mcp/stargazers) [![Forks](https://img.shields.io/github/forks/atilaahmettaner/tradingview-mcp?style=flat-square&color=blue)](https://github.com/atilaahmettaner/tradingview-mcp/network) [![Language](https://img.shields.io/badge/lang-Python-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-85%2F100-brightgreen?style=flat-square)](#)

> TradingView MCP server — real-time market data, technical analysis, screeners & backtesting for Claude, ChatGPT, Cursor & any MCP client. Stocks, crypto, forex & futures across global exchanges. Hosted or self-host.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 3.3k |
| 🍴 **Forks** | 705 |
| 💻 **Language** | Python |
| 📈 **Score** | 85/100 |
| 🗓️ **Last push** | 2026-06-29 |
| 🔍 **Source** | github |

## 🏷️ Topics

`claude-desktop` `cryptocurrency` `futures` `market-data` `mcp-server` `mcp-tools` `model-context-protocol` `openclaw` `stock-market` `technical-analysis` `trading-mcp` `trading-strategies`

## 🎯 Categories

Crypto · Trading · MCP · Backend · DevTools

## 📝 Summary

### English

atilaahmettaner/tradingview-mcp: atilaahmettaner/tradingview-mcp helps prototype or inspect blockchain workflows with open implementation details.. Use it for build Web3 workflows; inspect blockchain integrations. High for an OSS candidate: recent activity, adoption, and ecosystem signals are strong enough for a serious pilot.

### Русский

atilaahmettaner/tradingview-mcp: open-source проект в категориях Crypto, Trading, MCP, Backend, DevTools. Практическое применение: build Web3 workflows; inspect blockchain integrations. Уровень готовности: готов для серьезного pilot с проверкой license, security и maintainer activity.

### 中文

atilaahmettaner/tradingview-mcp：atilaahmettaner/tradingview-mcp helps prototype or inspect blockchain workflows with open implementation details.。适合用于build Web3 workflows、inspect blockchain integrations。High for an OSS candidate: recent activity, adoption, and ecosystem signals are strong enough for a serious pilot.

## 🧭 Practical evaluation

**Value:** atilaahmettaner/tradingview-mcp helps prototype or inspect blockchain workflows with open implementation details.

**Best use cases**

- build Web3 workflows
- inspect blockchain integrations
- prototype wallet or DeFi features

**Integration notes:** Looks straightforward to evaluate: it exposes implementation signals such as API/SDK/CLI, language metadata, or focused topics.

**Production readiness:** High for an OSS candidate: recent activity, adoption, and ecosystem signals are strong enough for a serious pilot.

**Quality signals**

- 3295 GitHub stars
- 705 forks
- updated 2026-06-29
- primary language: Python
- 13 topics

**Risks:** No major metadata risk found, but license, security posture, and active maintainers still need final review.

## 🧮 Score breakdown

| Dimension | Score |
|---|---:|
| usefulness | 90/100 |
| quality | 88/100 |
| integration | 78/100 |
| production | 84/100 |
| outlook | 91/100 |
| adoption | 74/100 |
| stars | 75/100 |
| forks | 71/100 |
| recency | 100/100 |
| topics | 100/100 |
| sourceTrust | 100/100 |

---

<sub>🔭 Discovered 2026-06-29 · [View on GitHub](https://github.com/atilaahmettaner/tradingview-mcp) · [← Back to Crypto](./README.md)</sub>
