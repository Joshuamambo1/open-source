# Webba-Creative-Technologies/vice

[![Stars](https://img.shields.io/github/stars/Webba-Creative-Technologies/vice?style=flat-square&color=yellow)](https://github.com/Webba-Creative-Technologies/vice/stargazers) [![Forks](https://img.shields.io/github/forks/Webba-Creative-Technologies/vice?style=flat-square&color=blue)](https://github.com/Webba-Creative-Technologies/vice/network) [![Language](https://img.shields.io/badge/lang-JavaScript-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-46%2F100-brightgreen?style=flat-square)](#)

> VICE is a security auditing CLI tool that finds vulnerabilities in your web applications.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 335 |
| 🍴 **Forks** | 17 |
| 💻 **Language** | JavaScript |
| 📈 **Score** | 46/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

AI/ML · DevTools · Security

## 📝 Summary

VICE is an open-source security auditing CLI tool designed to identify vulnerabilities in web applications, helping developers strengthen their online security. Utilizing its comprehensive scanning capabilities, VICE provides valuable insights and recommendations to improve the overall security posture of web applications. This tool is particularly useful for developers and security professionals seeking to proactively address potential security threats.

---

<sub>🔭 Discovered 2026-04-30 · [View on GitHub](https://github.com/Webba-Creative-Technologies/vice) · [← Back to AI/ML](./README.md)</sub>
