# suryast/indonesia-gov-apis

[![Stars](https://img.shields.io/github/stars/suryast/indonesia-gov-apis?style=flat-square&color=yellow)](https://github.com/suryast/indonesia-gov-apis/stargazers) [![Forks](https://img.shields.io/github/forks/suryast/indonesia-gov-apis?style=flat-square&color=blue)](https://github.com/suryast/indonesia-gov-apis/network) [![Language](https://img.shields.io/badge/lang-HTML-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-51%2F100-brightgreen?style=flat-square)](#)

> 🇮🇩 50+ Indonesian Government APIs & Data Sources — BPS, OJK, BPJPH, BPOM, Bank Indonesia, IDX, BMKG + MCP servers. Python examples, scraping patterns, and practical gotchas. It's very challenging to keep up with all the breaking and blocking changes of Indonesian government APIs and digital resources. Always verify and test rigorously.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 139 |
| 🍴 **Forks** | 12 |
| 💻 **Language** | HTML |
| 📈 **Score** | 51/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

AI/ML · Backend · DevTools

## 📝 Summary

Here's a brief summary of the project:

The indonesia-gov-apis project provides a comprehensive collection of over 50 Indonesian government APIs and data sources, including BPS, OJK, BPJPH, BPOM, Bank Indonesia, and more. This open-source repository offers Python examples, scraping patterns, and practical tips to help developers navigate the Indonesian government's digital resources. It is designed to be a valuable resource for those working with Indonesian government data, but users are advised to rigorously test and verify the APIs due to their frequently changing nature.

---

<sub>🔭 Discovered 2026-04-25 · [View on GitHub](https://github.com/suryast/indonesia-gov-apis) · [← Back to AI/ML](./README.md)</sub>
