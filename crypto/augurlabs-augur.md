# augurlabs/augur

[![Stars](https://img.shields.io/github/stars/augurlabs/augur?style=flat-square&color=yellow)](https://github.com/augurlabs/augur/stargazers) [![Forks](https://img.shields.io/github/forks/augurlabs/augur?style=flat-square&color=blue)](https://github.com/augurlabs/augur/network) [![Language](https://img.shields.io/badge/lang-Python-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-70%2F100-brightgreen?style=flat-square)](#)

> Python library and web service for Open Source Software Health and Sustainability metrics & data collection. You can find our documentation and new contributor information easily here: https://oss-augur.readthedocs.io/en/main/

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 695 |
| 🍴 **Forks** | 992 |
| 💻 **Language** | Python |
| 📈 **Score** | 70/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

Crypto · AI/ML · Data

## 📝 Summary

Augur is an open-source library and web service that provides comprehensive health and sustainability metrics for Open Source Software (OSS). It offers data collection and analysis capabilities, empowering developers and researchers to assess and improve the quality and reliability of OSS projects. Augur's metrics and data insights can be leveraged in various applications, including AI/ML and crypto-related projects.

---

<sub>🔭 Discovered 2026-05-01 · [View on GitHub](https://github.com/augurlabs/augur) · [← Back to Crypto](./README.md)</sub>
