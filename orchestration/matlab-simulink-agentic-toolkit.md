# matlab/simulink-agentic-toolkit

[![Stars](https://img.shields.io/github/stars/matlab/simulink-agentic-toolkit?style=flat-square&color=yellow)](https://github.com/matlab/simulink-agentic-toolkit/stargazers) [![Forks](https://img.shields.io/github/forks/matlab/simulink-agentic-toolkit?style=flat-square&color=blue)](https://github.com/matlab/simulink-agentic-toolkit/network) [![Language](https://img.shields.io/badge/lang-HTML-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-52%2F100-brightgreen?style=flat-square)](#)

> The Simulink Agentic Toolkit gives your AI agent both the tools and the expertise to work effectively with Simulink and Model-Based Design.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 170 |
| 🍴 **Forks** | 22 |
| 💻 **Language** | HTML |
| 📈 **Score** | 52/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

Orchestration · AI/ML · Backend

## 📝 Summary

The Simulink Agentic Toolkit is an open-source project that enables AI agents to effectively interact with Simulink and Model-Based Design, providing them with the necessary tools and expertise. This toolkit allows AI agents to integrate with Simulink models, facilitating seamless collaboration and automation in the design process. By leveraging the Simulink Agentic Toolkit, users can unlock new possibilities for AI-assisted modeling and simulation.

---

<sub>🔭 Discovered 2026-04-24 · [View on GitHub](https://github.com/matlab/simulink-agentic-toolkit) · [← Back to Orchestration](./README.md)</sub>
