# processing/p5.js

[![Stars](https://img.shields.io/github/stars/processing/p5.js?style=flat-square&color=yellow)](https://github.com/processing/p5.js/stargazers) [![Forks](https://img.shields.io/github/forks/processing/p5.js?style=flat-square&color=blue)](https://github.com/processing/p5.js/network) [![Language](https://img.shields.io/badge/lang-JavaScript-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-100%2F100-brightgreen?style=flat-square)](#)

> p5.js is a client-side JS platform that empowers artists, designers, students, and anyone to learn to code and express themselves creatively on the web. It is based on the core principles of Processing. Looking for p5.js 2.0? http://beta.p5js.org

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 23.6k |
| 🍴 **Forks** | 3.8k |
| 💻 **Language** | JavaScript |
| 📈 **Score** | 100/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

AI/ML · DevTools · Design

## 📝 Summary

Here's a brief summary of the p5.js project:

p5.js is an open-source, client-side JavaScript platform that enables users to learn coding and express creative ideas on the web. Based on the core principles of Processing, this platform empowers artists, designers, students, and beginners to bring their ideas to life. With p5.js, users can create interactive and dynamic web experiences with ease.

---

<sub>🔭 Discovered 2026-05-01 · [View on GitHub](https://github.com/processing/p5.js) · [← Back to AI/ML](./README.md)</sub>
