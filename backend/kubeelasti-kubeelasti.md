# KubeElasti/KubeElasti

[![Stars](https://img.shields.io/github/stars/KubeElasti/KubeElasti?style=flat-square&color=yellow)](https://github.com/KubeElasti/KubeElasti/stargazers) [![Forks](https://img.shields.io/github/forks/KubeElasti/KubeElasti?style=flat-square&color=blue)](https://github.com/KubeElasti/KubeElasti/network) [![Language](https://img.shields.io/badge/lang-Go-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-43%2F100-brightgreen?style=flat-square)](#)

> Kubernetes-native scale-to-zero with zero traffic loss, no code changes, and direct integration with kubernetes resources

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 234 |
| 🍴 **Forks** | 15 |
| 💻 **Language** | Go |
| 📈 **Score** | 43/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

Backend · DevOps/Infra

## 📝 Summary

KubeElasti is an open-source project that provides Kubernetes-native scale-to-zero capabilities with zero traffic loss, eliminating the need for code changes or manual scaling processes. This project enables seamless direct integration with Kubernetes resources, allowing for efficient and automated scaling strategies. By using KubeElasti, developers can easily manage and optimize their application's resources, reducing downtime and improving overall system reliability.

---

<sub>🔭 Discovered 2026-04-30 · [View on GitHub](https://github.com/KubeElasti/KubeElasti) · [← Back to Backend](./README.md)</sub>
