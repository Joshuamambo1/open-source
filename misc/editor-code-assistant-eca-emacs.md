# editor-code-assistant/eca-emacs

[![Stars](https://img.shields.io/github/stars/editor-code-assistant/eca-emacs?style=flat-square&color=yellow)](https://github.com/editor-code-assistant/eca-emacs/stargazers) [![Forks](https://img.shields.io/github/forks/editor-code-assistant/eca-emacs?style=flat-square&color=blue)](https://github.com/editor-code-assistant/eca-emacs/network) [![Language](https://img.shields.io/badge/lang-Emacs%20Lisp-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-49%2F100-brightgreen?style=flat-square)](#)

> Editor Code Assistant (ECA) integration for Emacs

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 406 |
| 🍴 **Forks** | 45 |
| 💻 **Language** | Emacs Lisp |
| 📈 **Score** | 49/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

Misc

## 📝 Summary

The ECA-Emacs project integrates the Editor Code Assistant (ECA) into the Emacs editor, providing users with a powerful code completion and suggestion tool. This open-source project enables Emacs users to leverage ECA's functionality, streamlining their coding experience and improving productivity. With ECA-Emacs, developers can enjoy intelligent code completion and suggestions directly within their Emacs environment.

---

<sub>🔭 Discovered 2026-04-30 · [View on GitHub](https://github.com/editor-code-assistant/eca-emacs) · [← Back to Misc](./README.md)</sub>
