# probabl-ai/skore

[![Stars](https://img.shields.io/github/stars/probabl-ai/skore?style=flat-square&color=yellow)](https://github.com/probabl-ai/skore/stargazers) [![Forks](https://img.shields.io/github/forks/probabl-ai/skore?style=flat-square&color=blue)](https://github.com/probabl-ai/skore/network) [![Language](https://img.shields.io/badge/lang-Python-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-55%2F100-brightgreen?style=flat-square)](#)

> Track your Data Science. Skore's open-source Python library accelerates ML model development with automated evaluation reports, smart methodological guidance, and comprehensive cross-validation analysis.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 637 |
| 🍴 **Forks** | 137 |
| 💻 **Language** | Python |
| 📈 **Score** | 55/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

AI/ML · Frontend · Data

## 📝 Summary

Here's a 2-3 sentence summary of the open-source project:

Skore is an open-source Python library that streamlines Machine Learning (ML) model development by providing automated evaluation reports, methodological guidance, and comprehensive cross-validation analysis. This tool enables data scientists to track their progress, identify areas for improvement, and optimize their models with ease. By leveraging Skore, developers can accelerate their ML workflows and make data-driven decisions with confidence.

---

<sub>🔭 Discovered 2026-04-29 · [View on GitHub](https://github.com/probabl-ai/skore) · [← Back to AI/ML](./README.md)</sub>
