# MoeKoeMusic/MoeKoeMusic

[![Stars](https://img.shields.io/github/stars/MoeKoeMusic/MoeKoeMusic?style=flat-square&color=yellow)](https://github.com/MoeKoeMusic/MoeKoeMusic/stargazers) [![Forks](https://img.shields.io/github/forks/MoeKoeMusic/MoeKoeMusic?style=flat-square&color=blue)](https://github.com/MoeKoeMusic/MoeKoeMusic/network) [![Language](https://img.shields.io/badge/lang-Vue-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-90%2F100-brightgreen?style=flat-square)](#)

> 一款开源简洁高颜值的酷狗第三方客户端 An open-source, concise, and aesthetically pleasing third-party client for KuGou that supports  Windows / macOS / Linux / Web :electron:

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 5.6k |
| 🍴 **Forks** | 366 |
| 💻 **Language** | Vue |
| 📈 **Score** | 90/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

Frontend · DevTools

## 📝 Summary

This project, MoeKoeMusic, is an open-source, user-friendly, and visually appealing third-party client for KuGou music platform. It supports multiple operating systems including Windows, macOS, Linux, and a web version. The client is designed to provide a seamless music streaming experience with a focus on simplicity and aesthetics.

---

<sub>🔭 Discovered 2026-04-25 · [View on GitHub](https://github.com/MoeKoeMusic/MoeKoeMusic) · [← Back to Frontend](./README.md)</sub>
