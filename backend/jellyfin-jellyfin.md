# jellyfin/jellyfin

[![Stars](https://img.shields.io/github/stars/jellyfin/jellyfin?style=flat-square&color=yellow)](https://github.com/jellyfin/jellyfin/stargazers) [![Forks](https://img.shields.io/github/forks/jellyfin/jellyfin?style=flat-square&color=blue)](https://github.com/jellyfin/jellyfin/network) [![Language](https://img.shields.io/badge/lang-C%23-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-95%2F100-brightgreen?style=flat-square)](#)

> The Free Software Media System - Server Backend & API

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 51.1k |
| 🍴 **Forks** | 4.7k |
| 💻 **Language** | C# |
| 📈 **Score** | 95/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

Backend

## 📝 Summary

Jellyfin is an open-source media server that provides a free and customizable alternative to commercial media solutions. It allows users to stream and manage their multimedia content, including videos, music, and images, across various devices and platforms. With its robust API and backend capabilities, Jellyfin offers a flexible and scalable solution for media management and sharing.

---

<sub>🔭 Discovered 2026-04-23 · [View on GitHub](https://github.com/jellyfin/jellyfin) · [← Back to Backend](./README.md)</sub>
